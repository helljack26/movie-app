...
const AddToWatchListButton = ({ name, image, genre, id, buttonType }) => {
    ...
    const [tooltipText, setTooltipText] = useState(buttonTypeArr.notInWatch.tooltip);
    switch (type === undefined ? buttonType : type) {
            ...
            return (
                <>
                    <button type='button' className={style.addIconBtn}>
                        <img src={buttonTypeArr.inWatch.url} alt={buttonTypeArr.notInWatch.tooltip}
                        ...
                        <div className={style.tooltip} style={{ left: '-150%' }}>
                            <p>{buttonType === true ? buttonTypeArr.inWatch.tooltip : tooltipText}</p>
                        </div>
                    </button>
                </>)
        case 'detailsInWatch':
        ...
        default:
            return <>
                <button type='button' className={style.addIconBtn}>
                    <img src={buttonTypeArr.notInWatch.url} alt="add to watch list"
                        className={`${style.addIcon} ${style.notInWatch}`} onClick={() => {
                            dispatch(toWatchList(name, image, genre, id))
                            setType('inWatch')
